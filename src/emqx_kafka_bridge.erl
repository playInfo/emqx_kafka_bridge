...
brod_init(_Env) ->
  ...
  {ok, ProducerConfig} = application:get_env(?APP, producer),
  Producer = [{default_producer_config, ProducerConfig}],
  KafkaConfig = lists:append(ClientConfig, Producer),
  io:format("Init EMQX-Kafka-Bridge with Broker: ~p, Config: ~w~n", [BootstrapBroker, KafkaConfig]),
  ok = brod:start_client(BootstrapBroker, brod_client_1, KafkaConfig).
